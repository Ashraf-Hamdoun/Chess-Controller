library chess_controller;

import 'package:chess_controller/src/constants/index.dart';
import 'package:chess_controller/src/utils.dart';
...
class Chess {
  ...
  bool load(String fen, {bool check_validity = true}) {
    ...
    for (var i = 0; i < position.length; i++) {
      ...
      if (piece == '/') {
        ...
        var color = (piece == piece.toUpperCase()) ? Color.WHITE : Color.BLACK;
    ...
    if (tokens[2].indexOf('K') > -1) {
      castling[Color.WHITE] |= BITS_KSIDE_CASTLE;
    }
    if (tokens[2].indexOf('Q') > -1) {
      castling[Color.WHITE] |= BITS_QSIDE_CASTLE;
    }
    if (tokens[2].indexOf('k') > -1) {
      castling[Color.BLACK] |= BITS_KSIDE_CASTLE;
    }
    if (tokens[2].indexOf('q') > -1) {
      castling[Color.BLACK] |= BITS_QSIDE_CASTLE;
    }
  ...
  String generate_fen() {
    ...
    for (var i = SQUARES_A8; i <= SQUARES_H1; i++) {
      if (board[i] == null) {
        ...
        fen += (color == Color.WHITE) ? type.toUpperCase() : type.toLowerCase();
      }
    ...
    var cflags = '';
    if ((castling[Color.WHITE] & BITS_KSIDE_CASTLE) != 0) {
      cflags += 'K';
    }
    if ((castling[Color.WHITE] & BITS_QSIDE_CASTLE) != 0) {
      cflags += 'Q';
    }
    if ((castling[Color.BLACK] & BITS_KSIDE_CASTLE) != 0) {
      cflags += 'k';
    }
    if ((castling[Color.BLACK] & BITS_QSIDE_CASTLE) != 0) {
      cflags += 'q';
    }
  ...
  Move build_move(List<Piece?> board, from, to, flags, [PieceType? promotion, String? comment]) {
    if (promotion != null) {
    ...
    return Move(turn, from, to, flags, board[from]!.type, captured, promotion, comment);
  }

  List<Move> generate_moves([Map? options]) {
    void add_move(List<Piece?> board, List<Move> moves, from, to, flags, [String? comment]) {
      /* if pawn promotion */
      if (board[from]!.type == PAWN &&
          (rank(to) == RANK_8 || rank(to) == RANK_1)) {
        const pieces = [QUEEN, ROOK, BISHOP, KNIGHT];
        for (var i = 0, len = pieces.length; i < len; i++) {
          moves.add(build_move(board, from, to, flags, pieces[i], comment));
        }
      } else {
        moves.add(build_move(board, from, to, flags, null, comment));
      }
    }
    ...
    second_rank[Color.BLACK] = RANK_7;
    second_rank[Color.WHITE] = RANK_2;
    ...
    for (var i = first_sq; i <= last_sq; i++) {
      ...
      if (piece.type == PAWN) {
        ...
        if (board[square] == null) {
          add_move(board, moves, i, square, BITS_NORMAL, null);

          /* double square */
          final square2 = i + PAWN_OFFSETS[us]![1];
          if (second_rank[us] == rank(i) && board[square2] == null) {
            add_move(board, moves, i, square2, BITS_BIG_PAWN, null);
          }
        }
        ...
        for (var j = 2; j < 4; j++) {
          ...
          if (board[square] != null && board[square]!.color == them) {
            add_move(board, moves, i, square, BITS_CAPTURE, null);
          } else if (square == ep_square) {
            add_move(board, moves, i, ep_square, BITS_EP_CAPTURE, null);
          }
        }
      } else {
    ...
    if ((!single_square) || last_sq == kings[us]) {
      ...
      if ((castling[us] & BITS_KSIDE_CASTLE) != 0) {
        ...
        if (board[castling_from + 1] == null &&
          ...
          add_move(board, moves, kings[us], castling_to, BITS_KSIDE_CASTLE, null);
        }
      }
      ...
      if ((castling[us] & BITS_QSIDE_CASTLE) != 0) {
        ...
        if (board[castling_from - 1] == null &&
            ...
            !attacked(them, castling_from - 1) &&
            !attacked(them, castling_to)) {
          add_move(board, moves, kings[us], castling_to, BITS_QSIDE_CASTLE, null);
        }
      }
    }
  ...
  List<String?> san_moves() {
    ...
    while (reversed_history.isNotEmpty) {
      ...
      if (pgn_move_number == start_move_number && move.color == Color.BLACK) {
  ...
  bool load_pgn(String? pgn, [Map? options]) {
    ...
    var ms_raw = pgn.replaceAll(header_string, '');

    // Tokenize the move section including comments, move numbers, and variations
    // Regex to match:
    // 1. Move number (e.g., "1.", "1...")
    // 2. Comments (e.g., "{ This is a comment }")
    // 3. Recursive Annotation Variations (RAV) (e.g., "(1... e5)")
    // 4. SAN move (any non-whitespace sequence)
    final tokenRegex = RegExp(r'(\d+\.{1,3})|({[^}]*})|(\([^\(]*\))|\S+');

    String? current_comment;
    String? current_result;

    // List to store the actual Move objects after parsing and before executing
    final List<Map<String, dynamic>> move_data_for_execution = [];

    for (final match in tokenRegex.allMatches(ms_raw)) {
        final token = match.group(0)!;
        final trimmed_token = trim(token);

        if (trimmed_token.isEmpty) continue; // Skip empty tokens

        if (RegExp(r'^\d+\.{1,3}$').hasMatch(trimmed_token)) {
            // It's a move number (e.g., "1.", "1...", "10.") - ignore for actual move processing
            continue;
        } else if (trimmed_token.startsWith('{') && trimmed_token.endsWith('}')) {
            current_comment = trimmed_token.substring(1, trimmed_token.length - 1).trim();
        } else if (trimmed_token.startsWith('(') && trimmed_token.endsWith(')')) {
            // It's a Recursive Annotation Variation (RAV), we currently ignore these for main line.
            // A more advanced parser would handle these.
            continue;
        } else if (POSSIBLE_RESULTS.contains(trimmed_token)) {
            current_result = trimmed_token;
            break; // Stop parsing moves once result is found
        } else {
            // This is a SAN move string
            move_data_for_execution.add({
                'move_string': trimmed_token,
                'comment': current_comment
            });
            current_comment = null; // Clear comment after associating it with a move
        }
    }

    // Now execute the moves
    for (final move_item in move_data_for_execution) {
        final move_string = move_item['move_string'] as String;
        final comment_for_move = move_item['comment'] as String?;

        final base_move = get_move_obj(move_string);

        if (base_move == null) {
            return false; // Invalid move, PGN load fails
        }

        // Create a new Move object with the base move details and the parsed comment
        final move_with_comment = Move(
            base_move.color,
            base_move.from,
            base_move.to,
            base_move.flags,
            ...
            base_move.captured,
            base_move.promotion,
            comment_for_move // Attach the comment here
        );
        make_move(move_with_comment);
    }

    // Finally, handle the result if one was found
    if (current_result != null && !header.containsKey('Result')) {
      set_header(['Result', current_result]);
    }
    return true;
  }
